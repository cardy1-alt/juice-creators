...
function SkeletonCard() {
  return (
    <div className="rounded-[12px] bg-white" style={{ boxShadow: '0 1px 4px rgba(42,32,24,0.04)' }}>
              ...
              <button key={c.id} onClick={() => onOpenCampaign(c.id)}
                ...
                <div className="flex-1 px-3.5 pb-3.5 pt-3 flex flex-col">
                  <div className="flex items-center gap-2 mb-2 min-w-0">
                    ...
                    <span className="text-[11px] text-[var(--ink-35)] truncate min-w-0">{c.businesses?.name}</span>
                  </div>
                  <p className="text-[16px] text-[var(--ink)] leading-[1.25] mb-1.5 line-clamp-2" style={{ fontWeight: 700 }}>{c.title || c.headline}</p>
                  {perkShort && <p className="text-[12px] text-[var(--ink-60)] line-clamp-1">{perkShort}{c.perk_value ? ` · £${c.perk_value}` : ''}</p>}
                  {c.expression_deadline && (
                    <p className="mt-auto pt-2 text-[11px] text-[var(--ink-35)]">Apply by {fmtDate(c.expression_deadline)}</p>
                  )}
                </div>
              </button>
            );
  ...
  return (
    <div className="px-4 md:px-6 lg:px-8 pb-8 pt-4 animate-fade-in">
      ...
      <div className="bg-white rounded-[12px] overflow-hidden" style={{ boxShadow: '0 1px 4px rgba(42,32,24,0.04)' }}>
        {items.map((item, i) => (
          <button key={i} onClick={item.action}
            className="w-full flex items-center gap-3 px-4 py-3.5 text-left hover:bg-[var(--shell)] transition-colors border-b border-[rgba(42,32,24,0.06)] last:border-0 min-h-[44px]">
      ...
      <button onClick={() => setShowSignOutConfirm(true)}
        className="w-full mt-4 flex items-center gap-3 px-4 py-3.5 bg-white rounded-[12px] hover:bg-[var(--shell)] min-h-[44px]" style={{ boxShadow: '0 1px 4px rgba(42,32,24,0.04)' }}>
