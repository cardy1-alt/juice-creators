...
export default function BusinessPortal() {
  ...
  // Approval gate — unapproved brands see a pending screen (can't create campaigns)
  if (!brand.approved) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center" style={{ background: 'var(--chalk)' }}>
        ...
        <p style={{ fontSize: 15, color: 'var(--ink-60)', lineHeight: 1.6, maxWidth: 380, marginBottom: 24 }}>
          We're reviewing <span className="font-medium text-[var(--ink)]">{brand.name}</span> and will email <span className="font-medium text-[var(--ink)]">{brand.owner_email}</span> once you're approved. Usually within 24 hours.
        </p>
        <div className="bg-white rounded-[12px] p-5 max-w-[420px] w-full text-left mb-6" style={{ boxShadow: '0 1px 4px rgba(42,32,24,0.04)' }}>
          <p className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--ink-50)] mb-3">What happens next</p>
          <div className="space-y-2.5">
            {[
              { n: 1, text: 'We verify your business to keep the marketplace trusted' },
              { n: 2, text: "You'll get an approval email with a link back to the portal" },
              { n: 3, text: 'Create your first campaign and start connecting with local creators' },
            ].map(s => (
        ...
        {selectedCampaignId && activeTab === 'campaigns' && campaignSubTab === 'summary' && campaign && (() => {
          ...
          <div>
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center gap-1.5 text-[14px] text-[var(--ink-60)]">
            ...
            <div className="bg-white rounded-[12px] p-4 md:p-5 mb-6" style={{ boxShadow: '0 1px 4px rgba(42,32,24,0.04)' }}>
              ...
              <button onClick={() => setShowPreview(true)}
                className="text-[14px] font-medium text-[var(--terra)] hover:underline">
                Preview as creator →
              </button>
            </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-4 flex-wrap">
              <select value={filterLevel} onChange={e => setFilterLevel(e.target.value)}
                className="px-3 py-2 rounded-[10px] bg-white text-[14px] text-[var(--ink)]" style={{ boxShadow: '0 1px 4px rgba(42,32,24,0.04)' }}>
              ...
              {selectedCreators.size > 0 && (
                <>
                  <span className="text-[14px] text-[var(--ink-60)] font-medium">{selectedCreators.size} selected</span>
                  <button onClick={handleBulkSelect}
                    className="min-h-[44px] px-4 py-2 rounded-[10px] bg-[var(--terra)] text-white text-[14px] hover:opacity-[0.90]" style={{ fontWeight: 700 }}>
                    Select {selectedCreators.size}
                  </button>
                  <button onClick={handleBulkDecline}
                    className="min-h-[44px] px-3 py-2 rounded-[10px] text-[14px] font-medium text-[var(--ink-60)] hover:text-[var(--ink)] hover:bg-[rgba(42,32,24,0.04)]">
                    Decline
                  </button>
                </>
              )}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <a href={`https://instagram.com/${handle}`} target="_blank" rel="noopener noreferrer"
                            className="text-[14px] text-[var(--terra)] hover:underline flex items-center gap-1">
