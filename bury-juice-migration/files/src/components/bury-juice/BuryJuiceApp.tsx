...
// The Bury Juice surface mounts at /bury on sponsor.juicelocal.co.uk.
// Admin management lives in the Juice Local AdminDashboard's
// "Bury Juice" tab. Two public routes: the storefront itself and
// the post-payment success screen.

type Route = { kind: 'storefront' } | { kind: 'success' };

function parseRoute(pathname: string): Route {
  const path = pathname.replace(/\/+$/, '') || '/';
  if (path === '/bury/success') return { kind: 'success' };
